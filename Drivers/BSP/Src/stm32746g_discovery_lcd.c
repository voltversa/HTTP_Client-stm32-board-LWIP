...
/**
 * @brief WDA_LCD_DrawBitmap is not part of the BSP it has been added to make it easier for Embedded Systems students
 * To place an image on the screen.
 *
 * @param: pSrc is a pointer to the image data. This can be an array created with the program "ConvertPictureDiscovery"
 * @param: xPos start position of the image (in pixel coordinates from the left of the screen)
 * @param: yPos start position of the image (in pixel coordinates from the top of the screen)
 * @param: xSize size of the image (in pixels)
 ...
 * @param: ySize size of the image (in pixels)
 * @param: ColorMode is the color format of the image data (pSrc).
 * @param: ImageWidth is the width of the image from which a piece is to be copied
 * @retval None
 */
void WDA_LCD_DrawPartOfBitmap(const void *pSrc, uint32_t xPos, uint32_t yPos, uint32_t xSize, uint32_t ySize, uint32_t ColorMode, uint32_t ImageWidth)
{
